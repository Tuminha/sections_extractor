...
def extract_conclusion(xml_root: ET.Element, namespace: dict) -> str:
    ...
    for div in xml_root.findall('.//tei:div', namespace):
        ...
        if head is not None:
            ...
            # Check if the title indicates the start of the 'Conclusion' section
            if 'conclusion' in section_title:
                ...
                for elem in div.iter():
                    if elem.tag.endswith('p') or elem.tag.endswith('figDesc'):
                        if elem.text:
                            conclusion += elem.text + "\n"
                    if elem.tail:
                        conclusion += elem.tail.strip() + "\n"
                break
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = "/Users/franciscoteixeirabarbosa/Dropbox/Science in Dentistry APP/pdf_extractor/sections_extractor/conclusion_output"
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            output_file_path = os.path.join(OUTPUT_DIR, f'{pdf_file.replace(".pdf", "")}_conclusion.txt')
            with open(output_file_path, 'w', encoding='utf-8') as f:
                f.write(conclusion)
            print(f"Conclusion for {pdf_file} has been written to {output_file_path}")
